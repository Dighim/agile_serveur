	...
	$.ajax({
		...
		success : function(data, textStatus, jqXHR) {
			...
			for(var i=0; i<data.length; i++){
				...
				list = list + "<li>" + data[i].pseudo;
				if(id==table.crea){
					list = list + "<a class='cancel' href='#' id='removeU"+data[i].id+"'><img src='cancel.png' height='20' width='20' ></a>";
				}
				list = list  + "</li>";
			}
			list = list + "</ul>";
			...
			$(".cancel").click(function(){
				...
				desinscriptionWithId(table.idTable, this.id.replace("removeU",""));
				$("#afficheUneTable").remove();
				listTables();
			});
		},
...
function invite(table){ 
    if(table.etat != 1 && table.crea == id) return "<button id='invitation' class='btn btn-default'>Inviter</button>";
    return "";
}
...
function desinscriptionWithId(idTable, idUser){
	console.log("IdUser: "+idUser);
	$.ajax({
		type : 'DELETE',
		contentType : 'application/json',
		url : "/v1/table/"+idTable+"/desinscription/"+idUser,
		dataType : "json",
		success : function(data, textStatus, jqXHR) {
			console.log("desinscription faite");
		},
